# Overlay UI (refineUiOverlay.json)

`overlayUI` — это декларативный JSON, который управляет тем, *что именно показывается* в сгенерированном refine UI (какие ресурсы доступны в меню и какие поля видны в таблицах/формах).

Он используется фронтендом из `src/refine/PersonaContext.tsx`, который читает `src/refine/refineUiOverlay.json`.

## Где применяется

- **Выбор persona** (admin/doctor/patient и т.п.) — `src/refine/personaResolve.ts`
...
- `matchRoles?: string[]`
  - список фрагментов/имен realm roles Keycloak
  - логика матчинга: persona выбирается, если **хотя бы один** `matchRoles` элемент совпал с одной из realm roles пользователя (подстрочное совпадение допускается)
...
## Как выбирается persona (personaResolve)
...
Важно: если Keycloak не инициализирован/не работает, то persona будет определяться по URL/Storage/`defaultPersona`.
...
3. overlay НЕ умеет:
   - переименовывать колонки/лейблы
   - добавлять полноценные UI-фильтры по колонкам
   - реализовывать row-level security (ограничение записей по текущему пользователю)
   - управлять правами CRUD (например “просмотр без редактирования”) без доработок кода

## Практические советы

- Чтобы быстро проверить действие `hiddenFields`, выберите persona через `?persona=doctor` и откройте страницы ресурсов.
- Для “клиника: пациент и доктор видят разное” обычно достаточно:
  - whitelisting `resources` для каждой роли
  - скрытия sensitive полей через `hiddenFields`
- Если требуется:
  - переименование/лейблы,
  - дополнительные фильтры,
  - row-level security,
  - permissions на редактирование,
  то это потребует расширения frontend-кода (overlay будет лишь входными параметрами).
